	...
	_changeMore: function(target, settings, value) {
		...
		if (text.length > inst.settings.length + inst.settings.leeway) {
			...
			var html = '<span class="more-showing">' + text.substring(0, pos) +
				...
				'<a href="#" class="more-link" alt="' + inst.settings.moreText + '">' +
				inst.settings.moreText + '</a>';
			target.empty().append(html).find('a').click(function(event) {
				var link = $(this);
				link.text(link.text() == inst.settings.moreText ?
						inst.settings.lessText : inst.settings.moreText).
					siblings('.more-ellipsis,.more-hidden').toggle();
